...
meta<-read_q2metadata("sowmetadata.txt")
str(meta)
colnames(meta)[2] <- "Code"
colnames(meta)[3] <- "cratenumber"
colnames(meta)[4] <- "parity"
str(meta)
...
aov.evenness.parity = aov(pielou_evenness ~ parity, data=meta)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.evenness.parity)
...
meta$parity.ord = factor(meta$parity, c("1", "2", "4", "5", "6"))
levels(meta$parity.ord)

#Plot
boxplot(pielou_evenness ~ parity.ord, data=meta, ylab="Peilou evenness")

evenness_boxplot <- ggplot(meta, aes(parity.ord, pielou_evenness)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
...
evenness_summary <- meta %>% # the names of the new data frame and the data frame to be summarised
  group_by(parity.ord) %>%   # the grouping variable
...
evenness_se <- ggplot(evenness_summary, aes(parity.ord, mean_evenness, fill = parity.ord)) + 
  geom_col() + 
...
kruskal.test(faith_pd ~ parity, data=meta)
...
pairwise.wilcox.test(meta$faith_pd, meta$parity.ord, p.adjust.method="BH")

# Like evenness, we see that pd also increases with age.

#Plot
boxplot(faith_pd ~ parity.ord, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

faith_pd_boxplot <- ggplot(meta, aes(parity.ord, faith_pd)) + 
  geom_boxplot(aes(color = parity.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
...
faith_days <- ggplot(meta, aes(parity, faith_pd)) + 
  geom_point(aes(color = parity.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Shannon Entropy", x = "") 
ggsave("output/faith_days.png", faith_days, height = 3, width = 3)


shannon_entropy <- 
  ggplot(meta, aes(parity, shannon_entropy)) + 
  geom_boxplot(aes(color = parity)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Shannon Entropy", x = "") 
ggsave("output/shannon_entropy.png", shannon_boxplot, height = 3, width = 3)
...
shannon_days <- 
  ...
  theme(legend.title = element_blank()) +
  labs(y="Shannon Entropy", x = "") 
ggsave("output/shannon_entropy.png", shannon_entropy, height = 3, width = 3)

  

observed_features_boxplot <- 
  ggplot(meta, aes(parity.ord, observed_features)) + 
  geom_boxplot(aes(color = parity.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
observed_features <- 
  ggplot(meta, aes(parity, observed_features)) + 
  geom_point(aes(color = parity.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
